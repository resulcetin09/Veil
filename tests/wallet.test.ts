...
import { discoverWallets, assertWalletNetwork, connectWallet } from '../src/lib/wallet';
...
describe('Wallet and private session', () => {
  it('ignores incompatible and malformed injected wallets', () => {
    ...
    expect(discoverWallets({ valid, legacy: { ...valid, apiVersion: '3.0' }, bad: {}, empty: null })).toEqual([valid]);
  });
  it('rejects a wrong network even if the wallet accepted the connection hint', async () => {
    const api = { getConnectionStatus: async () => ({ status: 'connected' }), getConfiguration: async () => ({ networkId: 'mainnet' }) } as unknown as ConnectedAPI;
    await expect(assertWalletNetwork(api)).rejects.toThrow('Preview');
  });
  it('does not consider a revoked connection valid', async () => {
    const api = { getConnectionStatus: async () => ({ status: 'disconnected' }) } as unknown as ConnectedAPI;
  ...
  it('propagates user rejection without fabricating a connected session', async () => {
    const wallet = { name: 'Lace', rdns: 'test', icon: '', apiVersion: '4.0.1', connect: async () => { throw new Error('User rejected'); } };
  ...
  it('scopes private state by contract and clears all session material', async () => {
    ...
    store.setContractAddress('01'.repeat(32));
    const copy = await store.get('guest'); copy!.invitationSecret![0] = 9;
