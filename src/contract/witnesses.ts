...
export const witnesses: Witnesses<PrivateState> = {
  organizerSecret: ({ privateState }) => {
    if (!privateState.organizerSecret)
      throw new Error("Import your organizer recovery file first.");
    return [privateState, privateState.organizerSecret];
  },
  invitationSecret: ({ privateState }) => {
    if (!privateState.invitationSecret)
      throw new Error("Import your private invitation first.");
  ...
  membershipPath: ({ ledger, privateState }, leaf) => {
    ...
    if (!path)
      throw new Error(
        "This invitation is not registered for this event. Contact the organizer.",
      );
