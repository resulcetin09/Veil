...
export function WalletModal({
  ...
  useEffect(() => {
    active.current = true;
    const interval = setInterval(() => setWallets(discoverWallets()), 2000);
    return () => {
  ...
  return (
    <Modal
      title={session ? "Your wallet" : "Make a private connection."}
      onClose={onClose}
    >
      {session ? (
        <>
          ...
          <p className="muted">
            Disconnecting clears invitations and organizer secrets from this
            tab. To revoke site permissions, use your wallet settings.
          </p>
        ...
        <>
          <p className="muted">
            Connect a Midnight wallet on Preview. Your wallet stays in control
          ...
          <div className="wallet-options">
            {wallets.map((wallet, index) => (
              <button
                ...
                key={`${wallet.rdns}-${index}`}
                disabled={busy}
                onClick={() => void connect(index)}
              >
                <span className="wallet-emblem">
          ...
          {!wallets.length && (
            <div className="wallet-empty">
              <Wallet size={38} weight="light" aria-hidden="true" />
              <h3>No compatible wallet detected</h3>
              <p>
                Install Lace with Midnight support, unlock it and select the
                Preview network.
              </p>
              <a
              ...
              <button
                ...
                onClick={() => setWallets(discoverWallets())}
              >
